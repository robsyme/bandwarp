...
- [Warp fit controls and manual row pinning](tickets/017-warp-fit-controls-and-pinning.md) — done (Operator feedback on drastic gels): tolerance and curve-stiffness sliders on the Bands step, plus a pin brush that assigns clicked bands to a chosen Compound Row — or a brand-new row the auto-fit missed — with pinned-row curves re-regressed through the pins; pins and fit settings persist in the Analysis File.
